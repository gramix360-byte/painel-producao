(()=>{
  ...
  function ensureForm(){
    ...
    $('#sales-channel')?.addEventListener('change',syncPaymentUI);
  ...
  function syncPaymentUI(){const channel=$('#sales-channel')?.value||'venda_externa',market=['mercado_livre','shopee','tiktok_shop'].includes(channel),total=Number($('#order-total-amount')?.value||0),status=market?'pago':'parcial',paid=market?total:Math.round(total*50)/100;if($('#payment-status')){$('#payment-status').value=status;$('#payment-status').disabled=true}if($('#amount-paid'))$('#amount-paid').value=paid.toFixed(2);$('.payment-partial-wrap')?.classList.toggle('hidden-payment',market);const hint=$('#payment-rule-hint');if(hint)hint.innerHTML=market?`<b>Pagamento pela plataforma</b><span>O pedido será registrado como totalmente pago.</span>`:`<b>Pagamento 50% + 50%</b><span>Entrada: ${money(paid)} · Saldo na entrega: ${money(total-paid)}</span>`}
  function captureForm(){const total=Number($('#order-total-amount')?.value||0),channel=$('#sales-channel')?.value||'venda_externa',market=['mercado_livre','shopee','tiktok_shop'].includes(channel),status=market?'pago':'parcial';formState={sales_channel:channel,payment_status:status,payment_method:$('#payment-method')?.value||'pix',amount_paid:market?total:Math.round(total*50)/100}}
