...
function getAll(name) {
  return reqPromise(store(name).getAll());
}
function getOne(name, key) {
  return reqPromise(store(name).get(key));
}
function put(name, value) {
  return reqPromise(store(name, "readwrite").put(value));
}
function del(name, key) {
  return reqPromise(store(name, "readwrite").delete(key));
}
...
async function nextOrderNumber() {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/next_order_number`, {
    method: "POST",
    headers: apiHeaders(),
    body: "{}",
  });
...
async function remotePushOrder(order) {
  ...
  const orderPayload = {
    ...
    notes: order.notes || "",
    status: order.status,
    phase: order.phase || "pedido_recebido",
  ...
  if (order.items?.length) {
    ...
    r = await fetch(`${SUPABASE_URL}/rest/v1/order_items`, {
      method: "POST",
...
async function pullRemote() {
  const r = await fetch(
    `${SUPABASE_URL}/rest/v1/orders?select=*,order_items(*)&deleted_at=is.null&order=created_at.desc`,
    { headers: apiHeaders() },
  );
  if (!r.ok) throw new Error(await r.text());
  const rows = await r.json(),
    pending = await pendingOrderIds(),
    remoteIds = new Set(rows.map((row) => row.id)),
    localOrders = await getAll("orders");
  for (const localOrder of localOrders)
    if (!remoteIds.has(localOrder.id) && !pending.has(localOrder.id))
      await del("orders", localOrder.id);
  for (const row of rows) {
    ...
    if (!local || new Date(row.updated_at) >= new Date(local.updated_at || 0))
      await put("orders", {
        ...
        notes: row.notes || "",
        status: row.status,
        phase: row.phase || "pedido_recebido",
...
async function syncNow(manual = false) {
  ...
  if (!navigator.onLine) {
    if (manual)
      toast("Sem internet. As alterações continuam salvas neste dispositivo.");
    updateConnection();
    return;
  }
  syncing = true;
  ...
  try {
    const outbox = await getAll("outbox"),
      ids = [...new Set(outbox.map((x) => x.orderId))];
    for (const orderId of ids) {
      const order = await getOne("orders", orderId);
      if (order) {
        await remotePushOrder(order);
        for (const entry of outbox.filter((x) => x.orderId === orderId))
          await del("outbox", entry.id);
      }
    }
    await pullRemote();
...
async function updateConnection() {
  const badge = $("#connection-badge"),
    ...
    pending = (await getAll("outbox")).length;
  $("#pending-badge").textContent =
    `${pending} pendente${pending === 1 ? "" : "s"}`;
  $("#pending-badge").classList.toggle("hidden", pending === 0);
  if (syncing) {
    badge.textContent = "Sincronizando...";
    badge.className = "connection syncing";
...
async function renderDashboard(orders, pending) {
  const today = new Date().toISOString().slice(0, 10),
    finishedToday = orders.filter(
      (o) => o.status === "finalizado" && o.finished_at?.slice(0, 10) === today,
    ).length,
    active = orders
  ...
  $("#view-dashboard").innerHTML =
    `<div class="stats"><div class="stat"><span>Aguardando</span><strong>${orders.filter((o) => o.status === "aguardando").length}</strong></div><div class="stat"><span>Em produção</span><strong>${orders.filter((o) => o.status === "em_producao").length}</strong></div><div class="stat"><span>Finalizados hoje</span><strong>${finishedToday}</strong></div><div class="stat"><span>Urgentes</span><strong>${orders.filter((o) => o.priority === "urgente" && o.status !== "finalizado").length}</strong></div></div><div class="panel"><div class="section-head"><h2>Fila atual</h2><span>${active.length} pedido(s)</span></div>${active.length ? active.map((o) => `<div class="order-list-row"><div class="order-number">#${escapeHtml(o.order_number)}</div><div class="order-list-main"><strong>${escapeHtml(o.customer_name)}</strong><div>${o.items.length} item(ns) · ${formatDate(o.due_date)}</div></div><span class="badge ${o.status}">${statusLabel(o.status)}</span>${pending.has(o.id) ? '<span class="badge pending">Pendente</span>' : ""}</div>`).join("") : '<div class="empty">Nenhum pedido na fila.</div>'}</div>`;
}
    ...
    const q = $("#search-orders").value.trim().toLowerCase(),
      ...
      list = orders.filter(
        (o) =>
          ...
          (s === "todos" || o.status === s) &&
          (p === "todas" || o.priority === p),
      );
    $("#orders-results").innerHTML = list.length
      ? list
          .map(
            (o) =>
              `<div class="order-list-row"><div class="order-number">#${escapeHtml(o.order_number)}</div><div class="order-list-main"><strong>${escapeHtml(o.customer_name)}</strong><div>${o.items.length} item(ns) · ${formatDate(o.due_date)}</div></div><span class="badge ${o.priority}">${o.priority}</span><span class="badge ${o.status}">${statusLabel(o.status)}</span>${pending.has(o.id) ? '<span class="badge pending">Pendente</span>' : ""}<button type="button" class="button danger order-delete" data-id="${o.id}">Excluir</button></div>`,
          )
          .join("")
      : '<div class="empty">Nenhum pedido encontrado.</div>';
  };
  ["#search-orders", "#filter-status", "#filter-priority"].forEach((s) =>
    $(s).addEventListener("input", render),
  );
  $("#orders-results").addEventListener("click", (event) => {
    const button = event.target.closest(".order-delete");
    if (button) deleteOrder(button.dataset.id);
  });
  render();
}
async function deleteOrder(id) {
  ...
  if (
    !confirm(
      `Excluir o pedido #${order.order_number}?\n\nEle será enviado para a lixeira e poderá ser restaurado nas Configurações.`,
    )
  )
    return;
  if (!navigator.onLine) {
    toast("Conecte-se à internet para excluir este pedido com segurança.");
    return;
  }
  try {
    ...
    for (const entry of (await getAll("outbox")).filter(
    ...
    toast(`Pedido #${order.order_number} enviado para a lixeira.`);
    broadcast();
  } catch (error) {
    console.error(error);
    toast("Não foi possível excluir o pedido. Tente novamente.");
  }
}
async function renderSync() {
  const pending = (await getAll("outbox")).length,
    last = await getOne("meta", "lastSync");
  $("#view-sync").innerHTML =
    `<div class="panel"><div class="section-head"><h2>Sincronização</h2><button id="sync-now" class="button primary">Tentar sincronizar agora</button></div><div class="sync-grid"><div class="sync-card"><span>Conexão</span><strong>${navigator.onLine ? "Online" : "Offline"}</strong></div><div class="sync-card"><span>Alterações pendentes</span><strong>${pending}</strong></div><div class="sync-card"><span>Última sincronização</span><strong>${last?.value ? new Date(last.value).toLocaleString("pt-BR") : "Ainda não realizada"}</strong></div><div class="sync-card"><span>Modo offline</span><strong>Ativo</strong></div></div><p>Os pedidos ficam salvos neste dispositivo usando IndexedDB. Quando a internet volta, as alterações pendentes são enviadas automaticamente ao Supabase.</p></div>`;
  $("#sync-now").addEventListener("click", () => syncNow(true));
}
async function refreshAll() {
  if (!db) return;
  const orders = (await getAll("orders")).sort(
...
function bindStatusActions() {
  $$(".status-action").forEach((btn) =>
    btn.addEventListener("click", async () => {
      const order = await getOne("orders", btn.dataset.id);
      if (!order) return;
      order.status = btn.dataset.status;
      if (order.status === "em_producao") {
        order.phase = "em_producao";
      ...
      toast(
        navigator.onLine
          ? "Status atualizado."
          : "Status salvo offline. Será sincronizado depois.",
      );
...
async function createOrderFromForm(e) {
  ...
  let orderNumber;
  try {
    orderNumber = navigator.onLine
      ? await nextOrderNumber()
      : `OFF-${Date.now().toString().slice(-6)}`;
  } catch (err) {
    console.error(err);
    toast("Não foi possível gerar o número do pedido. Tente novamente.");
    return;
  }
  const t = nowIso(),
    order = {
  ...
  await saveLocal(order, true);
  const generated = orderNumber.startsWith("OFF-")
    ? "número será gerado quando sincronizar"
    : `pedido #${orderNumber}`;
  e.target.reset();
  ...
  toast(
    navigator.onLine
      ? `Cadastrado ${generated} com sucesso.`
      : `Pedido salvo offline; o ${generated}.`,
  );
...
function switchView(view) {
  ...
  const meta = {
    dashboard: ["Dashboard", "Visão geral da produção"],
    novo: ["Novo Pedido", "Cadastre um pedido para a fila"],
    producao: ["Painel de Produção", "Tela para TV e equipe de produção"],
    pedidos: ["Pedidos", "Histórico e pesquisa"],
    sync: ["Sincronização", "Online + offline"],
  }[view];
...
async function init() {
  ...
  window.addEventListener("online", () => {
    updateConnection();
    syncNow();
    toast("Internet voltou. Sincronizando pedidos...");
  });
  window.addEventListener("offline", () => {
    updateConnection();
    toast("Sem internet. Você pode continuar trabalhando normalmente.");
  });
  ...
  if (navigator.onLine) syncNow();
  setInterval(() => {
    if (navigator.onLine && !syncing) syncNow();
  }, 8000);
}
init().catch((err) => {
  console.error(err);
  toast("Não foi possível iniciar o sistema. Recarregue a página.");
});
