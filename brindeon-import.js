  ...
  window.fetch=async function(input,init){
    try{
      ...
      if(original.includes('/rest/v1/products?')){
        ...
        if(!u.searchParams.has('limit')&&(init?.method||'GET').toUpperCase()==='GET')u.searchParams.set('limit','100');
        input=u.toString();
      }else if(original.includes('/rest/v1/product_images?')){
        const u=new URL(original,location.href);
        if(!u.searchParams.has('limit')&&(init?.method||'GET').toUpperCase()==='GET')u.searchParams.set('limit','500');
        input=u.toString();
      }
